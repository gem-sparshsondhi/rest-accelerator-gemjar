...
public class UtilitySteps {
    ...
    @When("^User adds header \"(.*?)\" as \"(.*?)\"$")
    public void addHeadersToRequest(String key, String value) {
        UtilityFunctions.addHeader(key, value);
    }

    @When("^User adds headers$")
    public void addHeadersToRequest(DataTable dt) {
        List<Map<String, String>> headers = dt.asMaps();
        HashMap<String,String> headerPairs = new HashMap<>();
        for (Map<String, String> header : headers) {
            headerPairs.put(header.get("Name"), header.get("Value"));
        }
        UtilityFunctions.addHeaders(headerPairs);
    }
    ...
    public void test() throws Exception {
        ...
        req.setpath("/items");
        req.setMethod("get");
        Response res = ApiInvocation.handleRequest(req);
